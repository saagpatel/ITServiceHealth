...
def text(x, y, s, *, size=8, color=TEXT_PRIMARY, weight="normal",
         ha="left", va="center", family=FONT_FAMILY):
    ax.text(x, y, s, fontsize=size, color=color, fontweight=weight,
            ha=ha, va=va, family=family)


def node(x, y, w, h, title, subtitle=None, *, border=BORDER,
         fill=SURFACE, title_size=9, sub_size=7):
    ...
    ax.add_patch(patch)
    if subtitle:
        text(x + w / 2, y + h * 0.62, title,
             size=title_size, color=TEXT_PRIMARY, weight="bold", ha="center")
        text(x + w / 2, y + h * 0.28, subtitle,
             size=sub_size, color=TEXT_SECONDARY, ha="center")
    else:
...
# --- title banner ---------------------------------------------------------
text(5, 79.5, "Pulse — IT Service Health",
     size=18, color=TEXT_PRIMARY, weight="bold")
text(5, 76.2,
     "Production architecture · vendor status pages → React dashboard · v2 shipped · 276 tests",
     size=9, color=TEXT_SECONDARY)
text(105, 79.5, "Box IT · Internal",
     size=9, color=TEXT_MUTED, ha="right")
text(105, 76.2, "Landscape · 11 × 8.5 in",
     size=8, color=TEXT_MUTED, ha="right", family=FONT_MONO)

# thin rule under title
ax.add_patch(Rectangle((5, 74), 100, 0.15,
                       facecolor=BORDER_STRONG, edgecolor="none"))
...
ax.add_patch(FancyBboxPatch(
    (5, 61.5), 100, 9,
    boxstyle="round,pad=0.2,rounding_size=1.2",
    linewidth=1.0, edgecolor=ACCENT_DIM_RED, facecolor="#1a0f12",
))
...
ax.add_patch(FancyBboxPatch(
    (5, 6), 100, 9,
    boxstyle="round,pad=0.2,rounding_size=1.2",
    linewidth=1.0, edgecolor=BORDER_STRONG, facecolor=SURFACE_ELEV,
))
text(6.5, 12.6, "Observability",
     size=9, color=TEXT_PRIMARY, weight="bold")
...
for i, (name, desc) in enumerate(chip_defs):
    ...
    ax.add_patch(FancyBboxPatch(
        ...
        linewidth=1.0, edgecolor=BORDER_STRONG, facecolor=SURFACE,
    ))
    text(cx + chip_w / 2, obs_y + 2.7, name,
         size=8, color=TEXT_PRIMARY, weight="bold", ha="center")
    text(cx + chip_w / 2, obs_y + 1.1, desc,
         size=6.5, color=TEXT_MUTED, ha="center")
...
ax.add_patch(FancyBboxPatch(
    (col1_x, 34), col1_w, 22,
    boxstyle="round,pad=0.2,rounding_size=1.2",
    linewidth=1.2, edgecolor=BORDER, facecolor=SURFACE,
))
text(col1_x + col1_w / 2, 53.8, "Vendor status sources",
     size=9, color=TEXT_PRIMARY, weight="bold", ha="center")
text(col1_x + col1_w / 2, 51.8, "30 services · ~60 s cadence",
     size=7, color=TEXT_MUTED, ha="center", family=FONT_MONO)
vendor_lines = [
    ("Statuspage.io JSON", "15 services"),
    ("Slack Status API", "1 service"),
    ("Google Workspace feed", "2 services"),
...
for i, (label, count) in enumerate(vendor_lines):
    ...
    text(col1_x + col1_w - 1.2, ly + 0.4, count,
         size=7, color=TEXT_MUTED, ha="right", family=FONT_MONO)

# --- Column 2: poll orchestrator + normalizer -----------------------------
col2_x = 28
col2_w = 18
node(col2_x, 47.5, col2_w, 8,
     "Poll Orchestrator",
     "APScheduler · async httpx · 60 s interval",
     border=ACCENT_DIM_RED)
node(col2_x, 36, col2_w, 8,
     "Status Normalizer",
...
col3_x = 50
col3_w = 18
node(col3_x, 47.5, col3_w, 8,
     "Change Detector",
     "diff vs current DB row · writes status_events")
node(col3_x, 36, col3_w, 8,
...
ax.add_patch(FancyBboxPatch(
    (col4_x, 43), col4_w, 12.5,
    boxstyle="round,pad=0.2,rounding_size=1.2",
    linewidth=1.4, edgecolor=ACCENT_ALARM, facecolor="#1a0f12",
))
text(col4_x + col4_w / 2, 53.3, "Alert Quality Layer",
     size=9, color=TEXT_PRIMARY, weight="bold", ha="center")
text(col4_x + col4_w / 2, 51.5, "Slack Block Kit → #service-validation",
     size=7, color=TEXT_SECONDARY, ha="center")
aq_lines = [
...
for i, line in enumerate(aq_lines):
    text(col4_x + 1.2, 49.2 - i * 1.2, line,
         size=6.8, color=TEXT_SECONDARY)

node(col4_x, 33, col4_w, 7.5,
     "SQLite Writer",
     "WAL · aiosqlite pool · retention job")

# --- Column 5: API + React dashboard --------------------------------------
col5_x = 92
col5_w = 13
node(col5_x, 47.5, col5_w, 8,
     "FastAPI",
     "/api/services · /api/summary · /api/timeline")
node(col5_x, 36, col5_w, 8,
     "React Dashboard",
     "Exec · Engineer · PWA · recharts")
...
ax.add_patch(FancyBboxPatch(
    (dl_x, 18.5), dl_w, 11,
    boxstyle="round,pad=0.2,rounding_size=1.2",
    linewidth=1.0, edgecolor=BORDER, facecolor=SURFACE_ELEV,
))
text(dl_x + dl_w / 2, 27.5, "Data lifecycle",
     size=8.5, color=TEXT_PRIMARY, weight="bold", ha="center")
dl_lines = [
...
for i, line in enumerate(edge_lines):
    text(edge_x + 1.0, 25.2 - i * 1.4, line,
         size=6.8, color=TEXT_SECONDARY)
...
text(col4_x + col4_w + 0.7, 49, "→ Slack",
     size=7, color=ACCENT_ALARM, weight="bold", family=FONT_MONO)
...
tether = FancyArrowPatch(
    (col2_x + col2_w / 2, 61.3), (col2_x + col2_w / 2, 55.5),
    arrowstyle="-", linestyle=(0, (2, 2)),
    color=ACCENT_ALARM, linewidth=1.2,
)
ax.add_patch(tether)
text(col2_x + col2_w / 2 + 0.5, 58.5, "guards", size=7,
     color=ACCENT_ALARM, weight="bold", family=FONT_MONO)

# Observability tether: neutral dashed line spanning the full pipeline up
# to each processing stage
for cx in [col2_x + col2_w / 2, col3_x + col3_w / 2,
          col4_x + col4_w / 2, col5_x + col5_w / 2]:
    t = FancyArrowPatch(
        (cx, 15), (cx, 33),
        arrowstyle="-", linestyle=(0, (1, 2)),
        color=TEXT_MUTED, linewidth=0.8,
    )
    ax.add_patch(t)
text(18, 15.8, "instruments every stage →",
     size=7, color=TEXT_MUTED, family=FONT_MONO, weight="bold")

# --- Footer ---------------------------------------------------------------
ax.add_patch(Rectangle((5, 3.5), 100, 0.1,
...
text(5, 2.0,
     ...
     size=7, color=TEXT_MUTED, family=FONT_MONO)
text(105, 2.0, "docs/architecture-diagram/architecture.pdf",
     size=7, color=TEXT_MUTED, ha="right", family=FONT_MONO)
...
fig.savefig("docs/architecture-diagram/architecture.png", format="png",
            facecolor=BG_PAGE, dpi=200)
print(f"wrote {out}")
